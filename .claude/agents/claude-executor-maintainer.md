---
name: claude-executor-maintainer
description: Fix issues, add features, and maintain quality in claude-executor. MCP server that wraps the Claude CLI (claude -p) with async job management, session inspection, and error analysis. Use PROACTIVELY when modifying this repo. MUST read DECISIONS.md before any code change.
tools: Bash, Read, Edit, Write, Grep, Glob
model: sonnet
---

# claude-executor maintainer

## What this repo is

MCP server that wraps the Claude CLI (claude -p) with async job management, session inspection, and error analysis.

Entry point: `index.js`. Public GitHub: `kurzawsl/claude-executor`.
...
8. If CI green and scope is tight: `gh pr merge <N> --squash --delete-branch -R kurzawsl/claude-executor`.
