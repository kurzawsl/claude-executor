# claude-executor — Decisions Log

This file captures *why*, not *what*. Read before opening a PR.
Append new entries at the TOP. Keep entries ≤15 lines.

## 2026-04-23 — security: harden defaults — safe dangerouslySkipPermissions, sessionId
**Context**: Unspecified external risk or critic finding (see PR and git log for details). (commit 434a064)
...
**Context**: README lacked install instructions, examples, or badges; repo looked abandoned from the outside. (commit 1070b86)
...
**Context**: None of the MCP entry points surfaced uncaughtException/unhandledRejection. Rejected promises silently killed the process. (commit 013fede)
...
**Context**: Repo was private; going public required LICENSE, clean package.json, no committed secrets. (commit 24f2445)
...
## 2026-04-23 — ci: add GitHub Actions + unit tests for job management
**Context**: Repo had no CI before this commit. Every push was merged without automated verification. (commit a4eef35)
